...
func ModifyUserPasswordOrInfo(w http.ResponseWriter, r *http.Request){
	r.ParseForm()
	log.Info(r.Form)
	username := r.Form.Get("username")
	password := r.Form.Get("password")
	...
	if dal.ModifyUser(username, password, info){
...
func GetUserInfo(w http.ResponseWriter, r *http.Request){
	...
	success, data := dal.GetUserInfo(username)
	res := responseJson{Success:false, Data:data}
	resStr, _ := json.Marshal(res)
	log.Info(string(resStr))
	fmt.Fprint(w, string(resStr))
}
